...
const MODEL = process.env.MODEL?.toLowerCase().startsWith("gemini-")
  ? process.env.MODEL
  : DEFAULT_MODEL;
...
const USER_REQUEST = `Organize all files in "${INBOX_DIR}": sort each file by its type first, then by the year/month/day of its last-modified date. Do this now, do not ask for confirmation.`;
...
try {
  ...
  mcpClient.callTool = (async (params, options) => {
    ...
    const label =
      params.name === "move_files" && Array.isArray(args.moves)
        ? `${args.moves.length} file(s)`
        : JSON.stringify(args);
  ...
  console.log(
    `(${fileCount} file(s) found → maximumRemoteCalls = ${MAX_TOOL_CALLS})\n`,
  );
